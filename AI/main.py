...
class RealtimeTiming:
    ...
    # Layer 命名對應（舊→新）：A=per_frame, B=heavy, C=roi, D=detect, Light=light
    # drill-down 鏈：heavy.roi → Layer roi；roi.{detect_p1,detect_p2} → Layer detect
    # （wrapper p1/p2 拆 bucket 留 per-pass 變異；Layer detect 內部 binary 等
    # 為兩 pass 累計，總和 ≈ p1+p2）。light.curve 與 heavy.motion_curve 同函數
    # 但拆 bucket 保 per-tier 變異對比。
    LAYERS = {
        "per_frame":  ["shift", "heavy", "light", "ingest", "viz"],
        "heavy":      ["seg_predict", "roi", "motion_curve",
                       "excursion_sf", "pv_render"],
        "roi":        ["detect_p1", "enhance", "detect_p2", "select"],
        "detect":     ["binary", "candidates", "curve_fit", "label_loop"],
        "light":      ["blur", "curve"],
    }

    def report(self, loop_total: float) -> None:
        """印各層 stage 耗時（per_frame % loop；其餘 % parent layer）。"""
        heavy_total = self.totals.get("heavy", 0.0)
        light_total = self.totals.get("light", 0.0)
        roi_total = self.totals.get("roi", 0.0)
        denominators = {
            "per_frame": loop_total,
            "heavy":     heavy_total,
            "roi":       roi_total,
            "detect":    roi_total,    # detect 為 roi 子層，% roi 對齊 drill-down
            "light":     light_total,
        }
        descriptions = {
            "per_frame": "per-frame time (% loop)",
            "heavy":     "heavy internal breakdown (% heavy)",
            "roi":       "heavy.roi breakdown (% roi)",
            "detect":    "roi.detect internal, both passes (% roi)",
            "light":     "light internal (% light)",
        }
        ...
        for layer, stages in self.LAYERS.items():
            print(f"  Layer {layer}: {descriptions[layer]}")
            for s in stages:
                # heavy/light 兩條 path 都有 motion_curve；report 共用同總和（同函數）
                line = _line(s, denominators[layer])
                if line:
                    print(line)
...
def _run_light_frame(
    gray: np.ndarray, y_band, bundle: RunBundle,
    timing: Optional["RealtimeTiming"] = None,
) -> MotionCurveResult:
    """REALTIME light tier：沿用 cached y_band 只跑 motion_curve（不跑 paddle）。

    timing 非 None 時記錄 Layer light 子步驟（blur / curve）；None 零影響。
    """
    t0 = time.perf_counter()
    blurred = cv2.medianBlur(gray, 7)
    if timing is not None:
        timing.record("blur", time.perf_counter() - t0)
        t0 = time.perf_counter()
    mc = extract_motion_curve(
        image=blurred,
        y_range=y_band,
        config=bundle.motion_curve,
    )
    if timing is not None:
        timing.record("curve", time.perf_counter() - t0)
    return mc
...
def _run_realtime(
    ...
    n_skip = 0
    # Patch 27A：per-frame loop wallclock array（含 skip）→ schema 1.1 jsonl
    per_frame_ms: list = []
    ...
    with video_ctx as vw:
        ...
        for i in range(1, stop):
            ...
            if not (is_excursion and shift.found and shift.shift_px > 0):
                ...
                per_frame_ms.append((time.perf_counter() - frame_start) * 1000)
                continue
            ...
            if need_heavy:
                ...
                t = time.perf_counter()
                motion_curve = _run_light_frame(gray, cached_y_band, bundle, timing=timing)
    ...
    if bundle.viz.save_timing_record:
        from pathlib import Path as _P
        source_stem = _P(seq.source_path).stem if seq.source_path else "unknown"
        record_run(
            timing=timing,
            ...
            metadata={
                ...
                "stop_at": stop,
                "fps": fps,
                "heavy_n": timing.counts.get("heavy", 0),
                "light_n": timing.counts.get("light", 0),
                "skip_n": n_skip,
                "config": {
                    "realtime_seg_refresh_max_n": mf.realtime_seg_refresh_max_n,
                    "realtime_warmup_frames": mf.realtime_warmup_frames,
                    "realtime_shift_strategy": str(mf.realtime_shift_strategy),
                    "realtime_shift_min_confidence": mf.realtime_shift_min_confidence,
                    "curve_fit_maxfev": bundle.detection.curve_fit_maxfev,
                },
            },
