...
## 11. 上下文衛生（Context Hygiene）

精簡 session、降低每回合 token 的工作習慣。

| # | 規範 | 為何 |
|---|------|------|
| 1 | 優先 `@file` 拉參考，不依賴長上下文 | 只載入當下需要的；避免為陳舊上下文反覆付費 |
| 2 | 單一 session 維持單一任務線 | 主題混雜會脹大上下文、混淆優先序 |
| 3 | 偵測主題切換時，提醒使用者 `/clear` | 丟掉累積的無關上下文，新主題乾淨起步 |
| 4 | 自足子任務（檢索、靜態分析、批次比對等）交 Subagent | 平行 + 隔離上下文，主線不被脹大 |

> 與既有規範分工：任務完成固化見 §3.4 / §8；多主題拆文件見 §10；輸出精簡見 §5。本節只收「session/上下文層」的習慣。
...
| 2026-06-08 | v1.3 | + §11 上下文衛生（@file / 單一任務線 / 主題切換 /clear / Subagent 委派） | 補精簡工作流規範，降每 session token |
