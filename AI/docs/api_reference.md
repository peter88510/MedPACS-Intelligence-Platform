...
| 版本 | 0.13 |
| 最後更新 | 2026-06-04 |
| 校對對象 | `config/*.py`、`algorithm/**/*.py`、`input/**/*.py`、`visualization/**/*.py`、`tools/timing_report.py` |
| 狀態 | snapshot |
...
| `use_segment_label` | `bool` | `True` | `select_target` mask 來源：True=paddle pass1；False=古典 pass2（Patch 12A 從 `RunConfig` 合入）。**Patch 24A**：True 且 pass1 成功時觸發 `enhanced_search(skip_detect=True)` 跳過 pass2，省 ~25-30 ms/heavy frame；pass1 失敗自動 fallback 完整 pass2 |
...
| `save_realtime_video` | `bool` | `True` | REALTIME mp4 影片輸出（`output/realtime/{stem}_realtime.mp4`） |
| `save_realtime_canvas_png` | `bool` | `False` | REALTIME 每幀 canvas PNG（debug 用，檔名稀疏跳號正常）|
| `save_timing_record` | `bool` | `True` | REALTIME run 收尾 append 1 行 JSONL 到 `output/timing/runs.jsonl` |
...
| `rt_show_motion_curve` | `bool` | `True` | realtime canvas: 軌跡（video / PNG 共用）|
| `rt_show_peak_markers` | `bool` | `True` | realtime canvas: crest/trough markers |
| `rt_show_excursion_text` | `bool` | `True` | realtime canvas: excursion_cm 文字 |

> Track 對照：final / debug / realtime video / realtime PNG / timing record。前二全 mode 生效；後三僅 REALTIME。realtime video + PNG 共用 canvas（rt_show_* 同時影響兩種）。
...
### §2.12 `RealtimeTiming` + Layer 命名

`main.py`（profiling 用，不參與演算法計算）

| 欄位 | 型別 | 說明 |
|---|---|---|
| `totals` | `dict[str, float]` | per-stage 累計秒數 |
| `counts` | `dict[str, int]` | per-stage 累計次數 |
| `LAYERS` | `dict[str, list[str]]` | 5 層 × sub-stage 順序定義 |

**Layer 命名映射（Patch 23A 命名 refactor，舊→新對照）**：

| 新名 | 舊名 | 分母 (% of) | Sub-stages |
|---|---|---|---|
| `per_frame` | Layer A | loop_total | shift / heavy / light / ingest / viz |
| `heavy`     | Layer B | heavy total | seg_predict / roi / motion_curve / excursion_sf / pv_render |
| `roi`       | Layer C | roi total | detect_p1 / enhance / detect_p2 / select |
| `detect`    | Layer D | roi total | binary / candidates / curve_fit / label_loop（兩 pass 累計） |
| `light`     | Layer Light | light total | blur / curve |

drill-down 鏈：`per_frame.heavy` → Layer heavy；`heavy.roi` → Layer roi；`roi.{detect_p1, detect_p2}` → Layer detect（內部 sub-stages 為兩 pass 累計）；`per_frame.light` → Layer light。`heavy.motion_curve` 與 `light.curve` 同為 `extract_motion_curve`，分 bucket 保 per-tier 變異。

duck-typed param：`detect` / `enhanced_search` 接 `timing=None`，非 None 時呼叫 `timing.record(stage, dt)` 寫入累計；algorithm 層不 import RealtimeTiming（main → algorithm 單向依賴）。
...
| Function | Signature | Returns | 用途 |
|---|---|---|---|
| `detect` | `(image, config, use_segment=None, timing=None)` | `DetectionResult` | 主入口；`use_segment` 提供時走 paddle 路徑；`timing` 為 REALTIME Layer detect 計時（duck-typed，optional）。**Patch 25A**：內部 curve_fit 步驟在 `len(potential_regions) == 1` 時 short-circuit（直接抓 input idx + (top, bottom)），byte-identical |
...
| `enhanced_search` | `(image_gray, y_band, detection_config, roi_band_config, timing=None, skip_detect=False)` | `RoiSearchResult` | enhance + medianBlur + pass2 detect；`skip_detect=True`（Patch 24A）僅跑 enhance 保 viz，跳過 medianBlur + pass2，回 stub（target_binary=None / padded_mask 與 overlay 為 zeros）|
...
| `render_realtime_canvas` | `(frame_idx, image_color, state, is_warmup, warmup_total, cfg, excursion_cfg)` | `np.ndarray` | REALTIME 即時視窗 canvas：frame[i] 底 + global 軌跡右錨定；回傳 BGR array（PNG 與 mp4 共用） |
| `RealtimeVideoWriter` | `(output_path, fps, width, height)` context manager；`write` / `write_skip` / `write_placeholder` | — | canvas → mp4 incremental writer（imageio-ffmpeg libx264，skip 幀 carry-forward）|
| `record_run` | `(timing, loop_total, metadata, jsonl_path=…)` | `None` | 收尾 append 1 行 JSON 到 `runs.jsonl`；供 `tools.timing_report` 跨 run 聚合 |
...
| 2026-06-03 | 0.10 | §1.7 `save_realtime` 拆 `save_realtime_video` / `save_realtime_canvas_png` + 新增 `save_timing_record`；§2.12 新增 `RealtimeTiming` + Layer 命名映射（A/B/C/D/Light → per_frame/heavy/roi/detect/light）；§3.9 刪 `render_realtime_global`、`render_realtime_canvas` 改回傳 ndarray、加 `RealtimeVideoWriter` / `record_run`；`tools/timing_report.py` 加 `--dedup` 模式 | Patch 22（mp4 視覺化重組）+ Patch 23（Layer 命名 refactor + JSONL run record + aggregation script + dedup）|
| 2026-06-03 | 0.11 | §1.3 `use_segment_label` 補 Patch 24A 跳過 pass2 條件式註；§3.4 `enhanced_search` 簽名加 `skip_detect=False` 與 stub return 說明 | Patch 24A：use_segment_label=True 且 pass1 成功時跳過 pass2 detect（省 ~25-30 ms/heavy frame，roi total -22~30%）|
| 2026-06-04 | 0.12 | §3.3 `detect` 內部 §5 curve_fit 步驟加 Patch 25A short-circuit 註：`len(potential_regions) == 1` 時直接從 input 抓 best_idx + regions，跳過 curve_fit | Patch 25A：唯一候選 byte-identical 短路（curve_fit 在此情境下無 reject threshold、regions 直接抄 input；省 ~50 ms/call）|
| 2026-06-04 | 0.13 | `tools/timing_report.py` 大幅重設計：頂部加 Legend；§1 即時性評估（15 FPS budget + PASS/FAIL）；§5 Layer 層級驗證（Parent / Children / Self / 一致性 %）；各 layer 表加 Count / Total / Avg / N eff / 占層比例 + 條件式 ⚠️；§9 加 parent chain；全欄位中文化；CLI 加 `--fps` 旗標 | Patch 26（report 呈現優化：四點 + sub-patch A/B/C/D/E）|
