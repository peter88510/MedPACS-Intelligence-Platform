...
| 維度 | 內容 |
|---|---|
| **入口** | `enhanced_search(image_gray, y_band, detection_config, roi_band_config, timing=None, skip_detect=False)` |
| **輸入** | 原圖 + y_band + cfg |
...
| **Patch 24A 跳過條件** | `_run_single_frame` 算 `skip_pass2 = use_segment_label AND pass1.target_binary is not None` 傳給 `skip_detect`；True 時僅跑 enhance（給 viz `ROI_BAND_ENHANCED` overlay），跳 medianBlur + pass2 detect，回 stub（`detection.target_binary=None`、`padded_mask/overlay=zeros`）。實測省 25-30 ms/heavy frame、roi total -22~30% |
...
| 2026-06-03 | — | §5.2 簽名加 `timing` / `skip_detect`、補 Patch 24A 跳過條件 row | Patch 24A：use_segment_label=True 條件式跳過 pass2 |
