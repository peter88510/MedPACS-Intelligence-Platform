...
| 版本 | v1.1 |
| 最後更新 | 2026-06-03 |
| 適用 | 改 viz 行為時參考；想了解 debug 圖 / final overlay / REALTIME mp4 怎麼出 |

---

## §1 模組目標

集中所有 cv2 / PIL / matplotlib / imageio-ffmpeg 畫圖副作用，讓 algorithm 層保持純函式。

五條獨立 track（debug / final 全 mode；後三條僅 REALTIME mode）：

| Track | 用途 | 控制 | 輸出位置 | 渲染者 |
|---|---|---|---|---|
| **debug** | 每處理階段一張圖；給開發者看演算法內部 | `save_debug` + `debug_stages` 過濾 | `output/debug/{stage}/{i:04d}.png` | PipelineVisualizer.render_frame |
| **final** | 綜合成果圖，呈現給人看 | `save_final` + 3 個 element toggle | `output/final/{i:04d}.png` | PipelineVisualizer.render_frame |
| **realtime video** | REALTIME canvas 串成 mp4（imageio-ffmpeg libx264）；skip 幀 carry-forward 保時間軸 | `save_realtime_video`（預設 True）| `output/realtime/{stem}_realtime.mp4` | `RealtimeVideoWriter` |
| **realtime PNG** | REALTIME 每幀 canvas（debug 用，預設關）| `save_realtime_canvas_png`（預設 False）| `output/realtime/canvas/{i:04d}.png` | `render_realtime_canvas` + `cv2.imwrite` |
| **timing record** | REALTIME run 收尾 append 1 行 JSONL | `save_timing_record`（預設 True）| `output/timing/runs.jsonl` | `visualization.timing_record.record_run` |

`cfg.enabled = False`（**default**）→ render_frame 立刻 return；零 I/O。realtime video / PNG 共用同一張 canvas array（`rt_show_*` 同時影響兩種）。
...
visualization/
├── __init__.py
├── pipeline_visualizer.py     PipelineVisualizer 主入口 + _render_final（debug / final track）
├── global_window.py           render_global_final（GLOBAL_WINDOW mode 拼接 final overlay）
├── realtime.py                render_realtime_canvas（REALTIME 即時 canvas，回傳 ndarray）
├── realtime_video.py          RealtimeVideoWriter（canvas → mp4 incremental，imageio-ffmpeg）
├── timing_record.py           record_run（REALTIME timing → JSONL append）
...
### REALTIME mode 額外 API

```python
from visualization.realtime import render_realtime_canvas
from visualization.realtime_video import RealtimeVideoWriter
from visualization.timing_record import record_run

# 每 ingested 幀（heavy / light tier 都會呼叫）
canvas = render_realtime_canvas(
    frame_idx=i, image_color=color, state=rt_state,
    is_warmup=..., warmup_total=..., cfg=viz_cfg, excursion_cfg=...,
)
# 同源寫 PNG / video（rt_show_* 共用 canvas array）
if save_realtime_canvas_png: cv2.imwrite(...)
if vw is not None: vw.write(canvas)

# Run 結束時 append 1 行 JSONL
if save_timing_record:
    record_run(timing, loop_total, metadata, jsonl_path)
```
...
### REALTIME track（video / PNG / timing）

REALTIME mode 三條額外 track（debug / final 仍走 heavy 幀 `_run_single_frame` 共用）：

- **video**（預設開）：`RealtimeVideoWriter` context manager 把 canvas 一張張寫入 mp4；libx264 quality=8 / macro_block_size=1；yuv420p 偶數寬高需求由 `_pad_to_even` 處理；skip 幀 `write_skip()` carry-forward 上一張 canvas、pre-bootstrap `write_placeholder(color)` 補位 → 影片時長 ≈ 原錄製時間
- **PNG**（預設關）：每 ingested 幀一張 canvas PNG，檔名稀疏（跳號正常，反映 skip 模式）
- **timing record**（預設開）：`_run_realtime` 收尾 append 1 行 JSON 到 `output/timing/runs.jsonl`；schema_version=1.0；nested `{layer: {stage: {total_s, count, avg_ms}}}` payload；不寫 source_stem 之外的路徑（醫療資料 indirect leak 防範）

### 為何分這麼多條

- debug 用灰階襯底、跟著演算法視角；final 用彩色保留 DCM 原色標
- debug 是給開發人員、final 是給人看
- realtime video / PNG 共用 canvas array（一次 render、兩種輸出）；rt_show_* 同時影響
- timing record 純資料（無圖），讓 `tools/timing_report.py` 跨 run 聚合分析
...
| `DETECTION_PASS2_OVERLAY` | `detection_pass2_overlay` | `refined.padded_overlay` | BGR color-coded（**Patch 24A**：`use_segment_label=True` + pass1 成功時 pass2 被跳過 → overlay 為 zeros，這 stage 落地為黑圖；屬可接受退化）|
...
| `realtime_canvas_path(cfg, frame_idx)` | `{output_dir}/realtime/canvas/{i:04d}.png` |
| `realtime_video_path(cfg, source_path)` | `{output_dir}/realtime/{stem}_realtime.mp4`（filename 由 source DCM stem 推導）|
...
| `should_save_realtime_video(cfg)` | `cfg.enabled and cfg.save_realtime_video` |
| `should_save_realtime_canvas_png(cfg)` | `cfg.enabled and cfg.save_realtime_canvas_png` |
...
├── global_window.py           →   algorithm.multiframe.GlobalExcursionResult / info_display / io
├── realtime.py                →   algorithm.multiframe.RealtimeState / info_display / io
├── realtime_video.py          →   imageio-ffmpeg / cv2 / numpy
├── timing_record.py           →   (純 stdlib：json / datetime / pathlib)
...
| 2026-06-03 | v1.1 | §1 兩 track → 五 track（+ realtime video / canvas PNG / timing record）；§2 子結構加 `realtime.py` / `realtime_video.py` / `timing_record.py` / `global_window.py`；§3 加 REALTIME mode API 段；§4 改稱五 track + REALTIME track 設計；§5 PASS2_OVERLAY 加 24A skip 退化註；§8 io 加 `realtime_*_path` / `should_save_realtime_*` | Patch 22（mp4 視覺化重組）+ Patch 23（JSONL timing record）+ Patch 24A（pass2 skip → PASS2_OVERLAY 退化）|
