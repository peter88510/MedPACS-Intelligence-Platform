...
| `multiframe/` | Multi-frame 模式 dispatch（LEGACY / GLOBAL_WINDOW / REALTIME） | `get_*_indices()` / `run_global_window()` / `RealtimeState.ingest_frame()` / `estimate_shift()` |
...
from algorithm.multiframe import (
    get_legacy_frame_indices, get_keyframe_indices,
    run_global_window, estimate_shift, RealtimeState,
)
...
- **curve_fit short-circuit（Patch 25A）**：`len(potential_regions) == 1` 時跳過 `diaphragm_curve_fit()` 呼叫，直接從 input 抓 `best_idx + (top, bottom)`。byte-identical（curve_fit 在單候選情境下無 reject threshold、`regions` 回傳值本來就是抄 input）；省 ~50 ms/heavy frame curve_fit 重複工作
- **fallback**：curve_fit 沒選出 best 時（或 candidates 為空），若有 use_segment_potentials 則取其 y 範圍 union，否則回 `(fallback_region_top, image_height)`
...
1. `enhance_band(image, y_band, num_segments=1)` → 變分增強 band 區域（**Patch 24A：無論 skip 與否都跑，給 viz overlay**）
...
**Patch 24A — 條件式跳過 pass2**：簽名加 `skip_detect: bool = False`。caller（`_run_single_frame`）算 `skip_pass2 = use_segment_label AND pass1.target_binary is not None` 傳入。`skip_detect=True` 時僅跑 step 1（enhance 給 viz 用）、跳過 step 2/3，回 stub `RoiSearchResult`（`detection.target_binary=None`、`padded_mask` / `padded_overlay` 為 zeros、`enhanced_padded` 仍正常）。實測同源對比：每 heavy frame 省 25-30 ms、roi total 降 22-30%。`use_segment_label=False` 或 pass1 fallback（target_binary=None）自動走完整 pass2 路徑。
...
| `REALTIME` | 逐幀 estimate_shift 變動位移累積 + rolling 全局 excursion；分層 cadence + mp4 video 輸出 | ✅ 14A-18 + 19A 分層 + 22 mp4（`RealtimeState.ingest_frame`）|
...
#### REALTIME 分層 cadence（Patch 19A）

estimate_shift 前移 → 無效幀（shift ≤ 0 / 低信心）直接跳過，省 paddle：

- **heavy tier**（paddle 整張 frame）：bootstrap / 峰觸發（piggyback brightness_way crest/trough x）/ 距上次 heavy ≥ `realtime_seg_refresh_max_n`
- **light tier**（多數幀）：沿用 cached y_band 只跑 motion_curve（不跑 paddle）
- mask 採 refresh-recent：heavy 幀整張 frame mask 右對齊覆蓋 buffer 最右 ~frame_width 欄；light 幀無 full_mask 傳入

#### REALTIME profiling（Patch 19/23A）— duck-typed cross-layer

`detect()` / `enhanced_search()` 接 `timing=None`（duck-typed，需 `record(stage, dt)`）；非 None 時記錄各 sub-stage 累計耗時。**algorithm 層不 import RealtimeTiming**（main → algorithm 單向依賴規約），dataclass 定義在 `main.py`，profiling 5 層命名：`per_frame` / `heavy` / `roi` / `detect` / `light`（見 [api_reference §2.12](../api_reference.md#212-realtimetiming--layer-命名)）。
...
- REALTIME 分層 cadence 是「省 paddle」優化，不改變 ingest 結果 — heavy / light 都走 `state.ingest_frame`，僅 mask 可選
...
| 2026-06-03 | v1.1 | §4.3 `enhanced_search()` 加 Patch 24A `skip_detect` 條件式跳過 pass2 段；§4.6 加「REALTIME 分層 cadence」「REALTIME profiling — duck-typed cross-layer」段；§2 / §3 加 `RealtimeState` / `estimate_shift` re-export | Patch 19A 分層 cadence + Patch 19/23A profiling + Patch 24A skip pass2 |
| 2026-06-04 | — | §4.2 加 curve_fit short-circuit（Patch 25A）說明 row | Patch 25A：len==1 byte-identical short-circuit |
