...
def detect(
    ...
    Args:
        ...
        timing: 可選計時累計器（duck-typed，需有 record(stage, dt)）。非 None 時記錄
                binary / candidates / curve_fit / label_loop（REALTIME Layer detect）；None 零影響
    ...
    # 5. Curve fit 挑最佳
    # Patch 25A：len==1 short-circuit — 唯一候選時 curve_fit 是 redundant 計算：
    #   - regions 回傳的 (top, bottom) 直接抄 input potential_regions 欄位（非 curve_fit 算）
    #   - score 無 reject threshold（score_min 起始 1e7，永遠 score < score_min）→ 必選之
    # 跳過 skeletonize/wavelet/scipy curve_fit ~50+ ms 重複工作；byte-identical
    best_idx: list = []
    regions: list = []
    if len(potential_regions) >= 1:
        if len(potential_regions) == 1:
            idx, top, bottom = potential_regions[0]
            best_idx, regions = [idx], [(top, bottom)]
        else:
