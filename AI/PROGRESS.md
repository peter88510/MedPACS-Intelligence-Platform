...
- Step 10 — Patch 22：REALTIME 視覺化重組（刪 `render_realtime_global` + canvas 改回傳 ndarray + 新 `RealtimeVideoWriter` imageio-ffmpeg libx264；cfg 拆 `save_realtime_video` / `save_realtime_canvas_png`；canvas → mp4 carry-forward / placeholder 保時間軸完整）
- Step 10 — Patch 23A：Layer 命名 refactor（A/B/C/D/Light → per_frame/heavy/roi/detect/light；sub-stage 與 layer 名對齊：heavy.roi → Layer roi、roi.detect_p* → Layer detect 兩 pass 累計、light.curve 與 heavy.motion_curve 分 bucket 保 per-tier 變異；行為 byte-identical 純命名）
- Step 10 — Patch 23B：JSONL run record（新 `visualization/timing_record.py`；`_run_realtime` 收尾 append 1 row 到 `output/timing/runs.jsonl`；schema_version=1.0；`save_timing_record` cfg flag 預設開）
- Step 10 — Patch 23C：aggregation script（新 `tools/timing_report.py`；CLI / 純 stdlib；§1 inventory + §2 ranking/outlier + §3-6 per-layer breakdown + §7 變異 top-N (CV) + §8 cadence + §9 趨勢 + §10 建議；N ≥ 5 才標可信）
- Step 10 — Patch 23D：SNAPSHOT 同步（api_reference 0.9 → 0.10：cfg 拆分 / §2.12 RealtimeTiming + Layer 命名映射 / §3.9 加 `RealtimeVideoWriter` + `record_run`；INDEX bump；`output/timing/*.jsonl` 由現有 `output/` ignore 涵蓋）
- Step 10 — Patch 23E：aggregator 加 `--dedup {all,last,first}`（預設 all 保留 event log 變異 sample；last/first 同 source_stem 去重 debug 重跑污染）；§1 inventory 顯示去重統計（原 N → kept N，dropped X）；寫入端不動
- 效能 — Patch 24A：條件式跳過 pass2 detect（`enhanced_search` 加 `skip_detect`；`use_segment_label=True` 且 pass1.target_binary 非 None 時觸發；pass2 mask 在此 cfg 下不被消費，跳過 medianBlur + pass2 detect，僅保 enhance 給 viz）；實測同源 Allen_Quiet_1 對照：roi total 2.42 s → 1.7-1.9 s（-22~30%），每 heavy frame 省 ~25-30 ms。fallback 保留：pass1 失敗或 `use_segment_label=False` 自動走完整 pass2
- Step 10 — Patch 24B：docs sync（api_reference 0.10 → 0.11 §1.3 `use_segment_label` 註 + §3.4 `enhanced_search` 簽名；pipeline.md §5.2 補跳過 row；INDEX bump；PROGRESS 紀錄）
- 文件治理 — STABLE tier 里程碑 sync（CLAUDE.md §10.5）：`README.md` v1.0 → v1.1（viz 5 track + Profiling 層 + tools/ + REALTIME 輸出 + `tools/timing_report.py` 用法）；`ARCHITECTURE.md` v1.0 → v1.1（§2 架構圖 + §3 模組責任 + §6.7-6.9 新設計決策：duck-typed timing / event-log JSONL / 條件式 skip pass2）；`modules/visualization.md` v1.0 → v1.1（兩 → 五 track + 子結構 + REALTIME API 段 + PASS2_OVERLAY 24A 退化註）；`modules/algorithm.md` v1.0 → v1.1（enhanced_search skip_detect + REALTIME 分層 cadence + REALTIME profiling）；INDEX bump 對應 4 份 STABLE
- 效能 — Patch 25A：detect 唯一候選 short-circuit 跳過 curve_fit（`len(potential_regions) == 1` 時 byte-identical 直接抓 input idx + (top, bottom)）；理由：curve_fit 在此情境下無 reject threshold（score_min 起始 1e7、score < score_min 必然成立）且 regions 回傳的 (top, bottom) 直接抄 input → 完整 skeletonize + wavelet + scipy curve_fit 是 redundant 50+ ms 重複計算
- Step 10 — Patch 25B：docs sync（api_reference 0.11 → 0.12 §3.3 detect 註；curve_fit.py docstring 補單一候選 caller 應 short-circuit 註；modules/algorithm.md §4.2 加 row；INDEX bump；PROGRESS 紀錄）
- Tooling — Patch 26：`tools/timing_report.py` 全面重設計（4 個 user 點 + 4 sub-patch + 1 docs）：
  - **26A 結構**：每 layer 表加「執行次數 μ / 總耗時 μ / 平均耗時 μ / 占層比例 μ / N eff / 條件式 ⚠️」六聯欄；§9 變異 top-N 加 parent chain 欄
  - **26B 中文化**：所有欄位 + 章節標題中文化；頂部 Legend 段一次定義 μ/σ/per-call/per-run/per-frame/占層比例/上層鏈/⚠️ 八個術語
  - **26C 層級驗證**：新加 §5 Layer Hierarchy Validation；Parent (來源) / Children sum / Self / 一致性% 對照；一致性 < 90% 標 ⚠️、> 105% 標 ❗
...
- **REALTIME profiling（Patch 19 timing → Patch 23A 改名）**：✅ per-stage 計時 instrumentation；5 層命名 per_frame/heavy/roi/detect/light（舊 A/B/C/D/Light）；duck-typed，None 零影響
