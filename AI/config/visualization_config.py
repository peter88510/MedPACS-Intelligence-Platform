...
| timing record  | `save_timing_record`       | `output/timing/runs.jsonl` (append)         | 僅 REALTIME | visualization.timing_record.record_run |
...
class VisualizationConfig:
    ...
    # REALTIME mode timing record append 到 output/timing/runs.jsonl（預設開，
    # 跨 run 變異分析資料源；單次 append ~1 行 JSON，對 per-frame 量測零影響）
    save_timing_record: bool = True
