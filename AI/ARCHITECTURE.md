...
```
main.py
  │  (orchestration: per-frame loop / multi-frame dispatch + RealtimeTiming profiling host)
  │
  ├── input/             DCM/PNG → FrameSequence
  │
  ├── algorithm/         per-frame & multi-frame pipeline
  │     ├── segmentation/         paddle 預測 → seg mask
  │     ├── diaphragm_detection/  mask + 古典法 → DetectionResult（duck-typed `timing` opt-in）
  │     ├── roi_band/             y_band 擴張 + enhanced search → RoiSearchResult + TargetSelection（24A: `skip_detect` 條件式跳過 pass2）
  │     ├── motion_curve/         軌跡擷取 + wavelet → MotionCurveResult
  │     ├── excursion/            peak/trough + 物理量 → ExcursionResult + PeakInfo
  │     ├── multiframe/           Multi-frame mode dispatch（LEGACY / GLOBAL_WINDOW / REALTIME）
  │     │     ├── frame_selection / global_window / realtime (RealtimeState) / frame_shift
  │     ├── signal_processing/    wavelet 等通用工具
  │     └── frame_result.py       FrameResult (per-frame 結果聚合)
  │
  ├── config/            per-layer dataclass configs
  │
  ├── visualization/     viz 五條獨立 track
  │     ├── pipeline_visualizer / layers/  debug + final（全 mode）
  │     ├── global_window.py              GLOBAL_WINDOW mode final overlay
  │     ├── realtime.py                   REALTIME canvas（回傳 ndarray）
  │     ├── realtime_video.py             RealtimeVideoWriter（canvas → mp4 imageio-ffmpeg）
  │     └── timing_record.py              record_run（REALTIME timing → JSONL）
  │
  └── tools/             CLI 工具
        └── timing_report.py              跨 run timing aggregation（JSONL → markdown report）
```
...
| `algorithm/multiframe/` | Multi-frame 模式（LEGACY / GLOBAL_WINDOW / REALTIME）dispatch | `get_*_indices()`、`run_global_window()`、`RealtimeState.ingest_frame()`、`estimate_shift()` |
| `algorithm/signal_processing/` | wavelet / skeleton helpers | `wavelet_denoising()` |
| `config/` | 全部 dataclass config + enum | `<Name>Config`、`Phase` / `MultiframeMode` / `KeyframeStrategy` / `ShiftStrategy` |
| `visualization/` | debug / final / realtime video / realtime PNG / timing record（5 條 track）| `PipelineVisualizer.render_frame()`、`render_realtime_canvas()`、`RealtimeVideoWriter`、`record_run()` |
| `tools/` | CLI 工具（runtime 不依賴）| `python -m tools.timing_report` 跨 run timing aggregation |
...
### §6.7 Duck-typed `timing` 跨層注入（Patch 19/23A）

**選擇**：REALTIME profiling 用 duck-typed `timing` 參數注入 algorithm 函式（`detect()`、`enhanced_search()`），而非 algorithm 反向 import 一個 profiling 模組。

**理由**：
- 維持 algorithm → visualization 單向依賴規約（§4）；`RealtimeTiming` dataclass 定義在 main.py
- algorithm 函式不需要知道 timing 實作；只要呼叫 `timing.record(stage, dt)`
- `timing=None` 預設零影響（沒有 wrapper 開銷）
- LEGACY / GLOBAL_WINDOW mode 不傳 timing 自動 no-op

**落實**：
- `detect(image, config, use_segment=None, timing=None)`
- `enhanced_search(..., timing=None, skip_detect=False)`
- 5 層命名 `per_frame` / `heavy` / `roi` / `detect` / `light` 對應 main.py `RealtimeTiming.LAYERS`

### §6.8 Event-log JSONL + 讀取端去重（Patch 23B/E）

**選擇**：REALTIME run timing 寫入端 append-only event log；去重 / 過濾在讀取端（`tools/timing_report.py --dedup`）做。

**理由**：
- 寫入端純粹：1 行 / 1 run，無 schema migration、無 race condition
- 重複 run 的「污染」是分析口徑問題不是資料污染（同源多次 run 對量「同源 run-to-run noise」有價值）
- `schema_version` 進 row 內，未來 LAYERS 改名舊資料仍可被讀
- 讀取端 `--dedup {all,last,first}` 對應不同分析意圖（量變異 / 留最新 / 留 baseline）

### §6.9 條件式跳過 pass2 detect（Patch 24A）

**選擇**：`enhanced_search` 加 `skip_detect: bool`；caller（`_run_single_frame`）算 `skip_pass2 = use_segment_label AND pass1.target_binary is not None` 傳入。

**理由**：
- `use_segment_label=True`（default）時，`select_target` 只用 pass1 mask，pass2 結果完全不被消費 → pass2 純粹是 ~36 ms 浪費
- 保留 enhance 步驟（0.5 ms）給 viz `ROI_BAND_ENHANCED` overlay；`DETECTION_PASS2_OVERLAY` 退化為黑圖（debug 用，可接受）
- pass1 失敗（target_binary=None）自動 fallback 完整 pass2 → 不失 robustness

**收益**：實測同源對照，每 heavy frame 省 25-30 ms；roi total 降 22-30%。
...
| 2026-06-03 | v1.1 | §2 架構圖更新（viz 5 track / tools/ / RealtimeTiming host）；§3 模組責任表加 `tools/` 與 viz 5 條 track；§6 加 §6.7 duck-typed timing / §6.8 event-log JSONL / §6.9 條件式跳過 pass2 三個設計決策 | Patch 19/22/23/24A 落地後架構層面回顧（duck-typed cross-layer profiling / event sourcing / pass2 short-circuit）|
