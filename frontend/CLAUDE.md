...
### 9.2 結構：A / B 兩段（與主 SESSION_HISTORY 對稱，2026-05-14 起）

```markdown
## A. 系統現況快照（必讀，每次 session 啟動）
### 系統現況          ← 一段話，前端目前狀態（檔案結構、套件版本、目前能做到的事）
### 進行中的任務      ← 與 PROGRESS 進行中段對應，但偏 session 視角
### 待決定事項        ← 前端視角的懸而未決（PROGRESS 不一定記）

## B. 工作脈絡（按需查閱）
### 已完成里程碑      ← 精選大事（避免與 PROGRESS「已完成任務」重複）
### 上次 session 結尾狀態  ← 改了什麼、commit 狀態、下次起手建議
```

**為何 A / B 分段**：A 段為「當下狀態」（小、必讀），B 段為「歷史脈絡」（大、按需）。前端 Agent 啟動只需讀 A、回顧時讀 B。降低每 session 啟動 token 消耗。
...
- 「結尾狀態」段每次必更新
- A / B 兩段結構不可破壞（除非主 Agent 通知結構升級）

### 9.5 主 Agent 對此檔的行為

- ✅ 可讀（理解前端目前 mental state、規劃下次 dispatch）
- ❌ **原則上不寫**
- ⚠️ **例外**（需在 commit message 明示）：
  1. 緊急修正過時資訊（如「commit 狀態」嚴重失準誤導下次 session）
  2. **結構性重組**（如本次 2026-05-14 A/B 拆段；只動結構、不動內容）
